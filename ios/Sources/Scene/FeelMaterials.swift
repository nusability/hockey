...
///
/// - **Chevron** — the aim ribbon: chevrons printed along it from its own shape (object space: x
///   across, z along 0…1, its width `Near + (Far − Near)·z`), one per 1/`Repeat` of its length,
///   scrolling outward on RealityKit's clock: `c = fract(z·Repeat − scroll·t)`, `s = 1 − |2u − 1|`,
...
enum FeelMaterials {
    ...
    private static func chevron() -> FxGraph {
        ...
        let colour = g.param("Colour", .color3), opacity = g.param("Opacity", .float), repeats = g.param("Repeat", .float)
        let near = g.param("Near", .float), far = g.param("Far", .float)
        let p = g.separate(g.position())
        let v = p[2]
        let width = g.add(near, g.mul(g.sub(far, near), v))
        let u = g.add(g.div(p[0], width), 0.5)
        let c = g.fract(g.sub(g.mul(v, repeats), g.mul(g.time(), Float(Presentation.Aim.scroll))))
        let centred = g.node("ND_absval_float", .float, [("in", .v(g.add(g.mul(u, 2), -1)))])[0]
