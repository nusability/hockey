...
final class RefusedScreen: Screen {
    init(game: Game, why: String) {
        ...
        let m = motion
        part(WaveText(L(.refusedTitle), height: 0.15, colour: C.coral, bob: 0.4, id: "refused_title_header", motion: m),
             at: at(0, top - 0.5))
        let card = part(Panel(size: [1.66, 1.2, 0.14], colour: C.cream, entrance: .tumble, motion: m), at: at(0, top - 1.45))
